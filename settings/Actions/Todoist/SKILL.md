...
**Script:** `$SKILL_DIR/scripts/todoist.py` (run with plain `python`).  
**Dependencies:** `todoist-api-python`, `thefuzz` (pre-installed in the image).
...
```bash
python /app/settings/Actions/Todoist/scripts/todoist.py search --on tomorrow
```
